 ...
 */
class OerJsonClient(apiKey: String) extends OpenExchangeRates {
	...
	private def getExchangeRates(downloadPath: String, currency: String): BigDecimal = {
		try {
			val url = new URL(oerUrl + downloadPath)
			val conn = url.openConnection()
			val node = mapper.readTree(conn.getInputStream())
			node.findValue(currency).getDecimalValue()
	    } catch {
