...
case class ForexLookupWhen(conversionAmount: Int, fromCurr: String, toCurr: String, fx: Forex) {
  ...
  * @returns currencyUnit representation of the currency, or OerErr if Joda money does not support the currency
  */
  private def convertToCurrencyUnit(currencyInStringRepresentation: String) : Either[OerErr, CurrencyUnit] = {
    try {
  ...
  * or OerErr with an error message containing the forex rate between the two currencies 
  * if either of the currency is not supported by Joda Money
  */
  private def returnMoneyOrJodaError(rate: BigDecimal) : Either[OerErr, Money] = {
    if (fromCurrencyUnit.isRight && toCurrencyUnit.isRight) {
      ...
      var errMessage = "Currency " + fromCurr + " and "+ toCurr + " can be found from the API " + 
                       "and " + fromCurr + ":" + toCurr + " = " + rate + " however" 
  ...
  * @param errObject - The OerErr object which contains the failure information obtained from API
  * @returns a Left of OerErr object which states the failure information 
  * and illegal currency info if there is any illegal currency
  */
  private def returnApiError(errObject : OerErr) : Either[OerErr, Money] = {
    var errMsg = "" 
    if (fromCurrencyUnit.isLeft) {
