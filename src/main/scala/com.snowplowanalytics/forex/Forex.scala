...
final case class ForexLookupWhen[F[_]](
  ...
  private def lookupNowishCache(
    ...
    val otherWay = OptionT(client.nowishCache.flatTraverse(cache => cache.get((toCurr, fromCurr)))).map {
      case (time, rate) => (time, inverseRate(rate))
    }
  ...
  private def lookupEodCache(
    ...
    val otherWay =
      OptionT(client.eodCache.flatTraverse(cache => cache.get((toCurr, fromCurr, eodDate)))).map(inverseRate)
